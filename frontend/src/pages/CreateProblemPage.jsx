...
function CreateProblemPage() {
  ...
  try {
    ...
    // Helper to clean and normalize newlines (\r\n, \n, \\n)
    ...
    const cleanData = {
      ...
      startCode: data.startCode.map(sc => ({
        ...sc,
        initialCode: normalizeText(sc.initialCode),
      })),
      referenceSolution: data.referenceSolution.map(rs => ({
        ...rs,
        completeCode: normalizeText(rs.completeCode),
      })),
    };
          ...
          {tabs.map((tab) => (
            <button
              ...
              className={`flex-1 px-4 py-3 rounded-lg font-medium transition-all ${
                activeTab === tab.id
                  ? "bg-gradient-to-r bg-blue-400 text-white shadow-lg"
